...
describe('IDP Controller', () => {
    it('Refresh - returns 422 when refreshToken is missing', async () => {
    ...
    it('Refresh - returns 500 for expired refresh token', async () => {
    ...
    it('Logout - returns 200 with token provided', async () => {
        const res = await request(app)
            .post('/api/idp/logout')
    ...
    it('Logout - returns 200 without token provided (current behavior)', async () => {
        const res = await request(app)
            .post('/api/idp/logout')
            .send({});
